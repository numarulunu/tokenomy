# firebreak — behavioral rules for Claude Code

These rules ship with the firebreak plugin. They nudge Claude toward token-efficient behavior. They do not override the user's own `CLAUDE.md` — user instructions always win.
...
- If firebreak's `read-once` hook blocks a re-read, do not retry — use the version already in your context.
- For log files, firebreak's `log-grep` hook filters to error lines + last 50. If you need the full log, include the token `!fulllog` in your next response or use `Bash: cat "$path"`.
...
1. `.claudeignore` — from firebreak's template (`${CLAUDE_PLUGIN_ROOT}/templates/.claudeignore`)
2. `.claude/settings.json` — from firebreak's template, with `respectGitignore: true`
